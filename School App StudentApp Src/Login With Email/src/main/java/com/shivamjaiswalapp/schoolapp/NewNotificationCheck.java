...
public class NewNotificationCheck extends Service {
    ...
    public void onCreate() {
        ...
        File file2 = new File(folder, "UserDetails_classCode.txt");
        String classCode = getdata(file2);


        db.collection("Notifications").document("Students").collection(classCode)
                .whereGreaterThan("timeSent",time)
                    ...
                    public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                        ...
                        if (value != null ){
                            for(DocumentSnapshot document : value.getDocuments()){
                                String time = getConvertedTime(document.getLong("timeSent"));
